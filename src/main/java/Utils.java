...
public class Utils {
    ...
    public static String formattimemsg(final long initTime, final long finalTime, final String mod) {
    	long minutes = ((finalTime-initTime) / 1000)  / 60;
		int seconds = (int) (((finalTime-initTime) / 1000) % 60);
		
		String msg;
		
		if (seconds > 0 || minutes > 0)
    		msg = mod + " " + minutes + "m " + seconds + "s";
		else 
			msg = mod + " " + (finalTime-initTime) + "ms";
		
		return msg;
    }
    
    
    public static String formatvertmsg(final ArrayList<Vector2> printers) {
    	String s = "";
		
		final ArrayList<Vector2> added = new ArrayList<>();
		
		for (Vector2 vert : printers) {
			boolean add = true;
			for (Vector2 check : added) {
				if (vert.approxequals(check)) {
					add = false;
					break;
				}
			}
